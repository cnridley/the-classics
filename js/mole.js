...
function moveMole() {
  let timerId = null;
  timerId = setInterval(randomSquare, 700); //set interval on so that the randomsquare function is called every second.
}

moveMole();

// added event listener to start button so that the timers only starts when it is clicked 
//timer ends when it reaches 0. 
